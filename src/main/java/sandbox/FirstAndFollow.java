...
public class FirstAndFollow {
    ...
    public Set<Symbol> first(final Symbol s) { return first.get(s); }

    /** @return follow(s) for the symbol s */
    public Set<Symbol> follow(final Symbol s) { return follow.get(s); }
                ...
                g.get(nt).forEach(p -> {
                    ...
                    reverse(p.getRhs()).forEach(b -> {
                        ...
                        if (b instanceof Symbol.NonTerminal) {
                            ...
                            if (!follow(b).containsAll(tail)) {
                                // add the items to follow(b_i)
                                follow(b).addAll(tail);
                            ...
                            // preceding b's.
                            if (first(b).contains(Symbol.ε)) {
                                final Set<Symbol> fb = new HashSet<>(first(b));
                                fb.remove(Symbol.ε);
                                tail.addAll(fb);
                            }
